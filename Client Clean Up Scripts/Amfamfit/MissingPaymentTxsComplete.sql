...
declare TxCursor Cursor for
		  select maipri.TxTransactionId as TxTransactionId, mair.TxInvoiceId, p.TargetDate, 
			 'ID: ' + convert(varchar(max),p.TxPaymentID) + ' | ' +
			...
			case Left(ppr.Token, 2) when 'BC' then right(ppr.token, 3) else right(ppr.token, 4) end + ' | Payment request due:' + convert(varchar(max), p.targetdate, 101) as Description, 
