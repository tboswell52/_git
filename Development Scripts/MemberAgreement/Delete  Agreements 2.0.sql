...
WHILE (@@fetch_status <> -1) 
	BEGIN
			...
			WHERE        (MemberAgreementItemPromotionEffectId IN
										 (SELECT        MemberAgreementItemPromotionEffectId
										   FROM            MemberAgreementItemPromotionEffect
										   WHERE        (MemberAgreementPromotionId IN
																		 (SELECT        MemberAgreementPromotionId
																		   FROM            MemberAgreementPromotion
																		   WHERE        (MemberAgreementId = @memAgrId)))))
			...
			DELETE FROM MemberAgreementPromotion
			WHERE        (MemberAgreementId = @memAgrId)
